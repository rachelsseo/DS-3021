...
# we need to normalize the numeric variables so that they're all on the same scale
...
# we can transform our categorical values by one-hot encoding them and using minMaxScaler
# categorical features tend to not be very helpful in kNN
...
# The function is joining the encoded data and the original data on columns
...
# function for today if you want to add it to your .knn.py file
#%%
...
# Prevalence is showing that we need a super efficient model - the true positive rate is going to be really high. 
# Will get the prediction right at
# The 
...
# more accurate when there's less k's - but suspicious because this means we're fitting it exactly to our train data
# because we one-hot encoded the target variable and dropped only `signed_up_0`
# so `signed_up_1` is our exact target variable of 'Trues'
#%%
...
# red flag model because the distribution is super bimodal with a low k value 
...
# if the threshold is 90% or above, count it as a 1 and if not, count it as a 0
#%%
